...
var textpattern = (function () {
  ...
  var $_1squqcqhjeajolh9 = { getPatterns: getPatterns };
  ...
  var patternFromRng = function (patterns, rng, space) {
    ...
    var endPattern = $_dzhrtcqpjeajolhk.findEndPattern(patterns, text, rng.startOffset, delta);
    ...
    var startPattern = $_dzhrtcqpjeajolhk.findPattern(patterns, patternRng.toString());
  ...
  var applyBlockFormat = function (editor, patterns) {
    ...
    if (textBlockElm) {
      ...
      if (firstTextNode) {
        pattern = $_dzhrtcqpjeajolhk.findPattern(patterns, firstTextNode.data);
        if (!pattern) {
          return;
        }
  ...
  function handleEnter(editor, patterns) {
    ...
    wrappedTextNode = $_1ycfrkqmjeajolhf.applyInlineFormatEnter(editor, patterns);
    if (wrappedTextNode) {
    ...
    $_1ycfrkqmjeajolhf.applyBlockFormat(editor, patterns);
  }
  function handleInlineKey(editor, patterns) {
    var wrappedTextNode, lastChar, lastCharNode, rng, dom;
    wrappedTextNode = $_1ycfrkqmjeajolhf.applyInlineFormatSpace(editor, patterns);
    if (wrappedTextNode) {
      dom = editor.dom;
  ...
  var setup = function (editor, patternsState) {
    ...
    editor.on('keydown', function (e) {
      if (e.keyCode === 13 && !VK.modifierPressed(e)) {
        $_gfo5ybqljeajolhc.handleEnter(editor, patternsState.get());
      }
    }, true);
    editor.on('keyup', function (e) {
      if ($_gfo5ybqljeajolhc.checkKeyCode(keyCodes, e)) {
        $_gfo5ybqljeajolhc.handleInlineKey(editor, patternsState.get());
      }
    });
    editor.on('keypress', function (e) {
      if ($_gfo5ybqljeajolhc.checkCharCode(charCodes, e)) {
  ...
  var $_7wkcpgqijeajolha = { setup: setup };

  PluginManager.add('textpattern', function (editor) {
    var patternsState = Cell($_1squqcqhjeajolh9.getPatterns(editor.settings));
    $_7wkcpgqijeajolha.setup(editor, patternsState);
    return $_bs5nlpqgjeajolh8.get(patternsState);
  });
  function Plugin () {
  }
