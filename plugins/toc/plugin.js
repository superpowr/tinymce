...
var toc = (function () {
  ...
  var $_2eh7f4qyjeajolik = { create: create };

  var tocId = $_2eh7f4qyjeajolik.create('mcetoc_');
  ...
  var readHeaders = function (editor) {
    var tocClass = $_52mfoiqxjeajolij.getTocClass(editor);
    var headerTag = $_52mfoiqxjeajolij.getTocHeader(editor);
    var selector = generateSelector($_52mfoiqxjeajolij.getTocDepth(editor));
  ...
  var generateTocHtml = function (editor) {
    ...
    return '<div class="' + editor.dom.encode($_52mfoiqxjeajolij.getTocClass(editor)) + '" contenteditable="false">' + html + '</div>';
  };
  var generateTocContentHtml = function (editor) {
    ...
    html += generateTitle($_52mfoiqxjeajolij.getTocHeader(editor), I18n.translate('Table of Contents'));
    for (i = 0; i < headers.length; i++) {
      h = headers[i];
      h.element.id = h.id;
  ...
  var register = function (editor) {
    editor.addCommand('mceInsertToc', function () {
      $_11tg6bqtjeajolie.insertToc(editor);
    });
    editor.addCommand('mceUpdateToc', function () {
      $_11tg6bqtjeajolie.updateToc(editor);
    });
  };
  var $_45bha1qsjeajolic = { register: register };

  var setup = function (editor) {
    var $ = editor.$, tocClass = $_52mfoiqxjeajolij.getTocClass(editor);
  ...
  var $_8210zdqzjeajolil = { setup: setup };

  var toggleState = function (editor) {
    return function (e) {
      var ctrl = e.control;
      editor.on('LoadContent SetContent change', function () {
        ctrl.disabled(editor.readonly || !$_11tg6bqtjeajolie.hasHeaders(editor));
      });
    };
  };
  var isToc = function (editor) {
    return function (elm) {
      return elm && editor.dom.is(elm, '.' + $_52mfoiqxjeajolij.getTocClass(editor)) && editor.getBody().contains(elm);
    };
  };
  ...
  var $_7rkpw0r0jeajolim = { register: register$1 };

  PluginManager.add('toc', function (editor) {
    $_45bha1qsjeajolic.register(editor);
    $_7rkpw0r0jeajolim.register(editor);
    $_8210zdqzjeajolil.setup(editor);
  });
  function Plugin () {
  }
