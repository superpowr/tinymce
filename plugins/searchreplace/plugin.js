...
var searchreplace = (function () {
  ...
  var $_g6pki3isjeajok0c = { findAndReplaceDOMText: findAndReplaceDOMText };
  ...
  var markAllMatches = function (editor, currentIndexState, regex) {
    ...
    return $_g6pki3isjeajok0c.findAndReplaceDOMText(regex, node, marker, false, editor.schema);
  };
  ...
  var get = function (editor, currentIndexState) {
    var done = function (keepEditorSelection) {
      return $_eq6leziqjeajok07.done(editor, currentIndexState, keepEditorSelection);
    };
    var find = function (text, matchCase, wholeWord) {
      return $_eq6leziqjeajok07.find(editor, currentIndexState, text, matchCase, wholeWord);
    };
    var next = function () {
      return $_eq6leziqjeajok07.next(editor, currentIndexState);
    };
    var prev = function () {
      return $_eq6leziqjeajok07.prev(editor, currentIndexState);
    };
    var replace = function (text, forward, all) {
      return $_eq6leziqjeajok07.replace(editor, currentIndexState, text, forward, all);
    };
    return {
      done: done,
      find: find,
  ...
  var open = function (editor, currentIndexState) {
    ...
    function updateButtonStates() {
      win.statusbar.find('#next').disabled($_eq6leziqjeajok07.hasNext(editor, currentIndexState) === false);
      win.statusbar.find('#prev').disabled($_eq6leziqjeajok07.hasPrev(editor, currentIndexState) === false);
    }
    ...
    var win = editor.windowManager.open({
      ...
      onClose: function () {
        editor.focus();
        $_eq6leziqjeajok07.done(editor, currentIndexState);
        editor.undoManager.add();
      },
      onSubmit: function (e) {
        ...
        if (!text.length) {
          $_eq6leziqjeajok07.done(editor, currentIndexState, false);
          win.statusbar.items().slice(1).disabled(true);
          return;
        }
        if (last.text === text && last.caseState === caseState && last.wholeWord === wholeWord) {
          if (!$_eq6leziqjeajok07.hasNext(editor, currentIndexState)) {
            notFoundAlert();
            return;
          }
          $_eq6leziqjeajok07.next(editor, currentIndexState);
          updateButtonStates();
          return;
        }
        count = $_eq6leziqjeajok07.find(editor, currentIndexState, text, caseState, wholeWord);
        if (!count) {
          notFoundAlert();
        }
      ...
      buttons: [
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            if (!$_eq6leziqjeajok07.replace(editor, currentIndexState, win.find('#replace').value())) {
              win.statusbar.items().slice(1).disabled(true);
              currentIndexState.set(-1);
              last = {};
            }
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_eq6leziqjeajok07.replace(editor, currentIndexState, win.find('#replace').value(), true, true);
            win.statusbar.items().slice(1).disabled(true);
            last = {};
          }
        },
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_eq6leziqjeajok07.prev(editor, currentIndexState);
            updateButtonStates();
          }
        },
        {
  ...
  var $_ak1neniujeajok0o = { open: open };

  var register = function (editor, currentIndexState) {
    editor.addCommand('SearchReplace', function () {
      $_ak1neniujeajok0o.open(editor, currentIndexState);
    });
  };
  var $_a6bjjcitjeajok0m = { register: register };
  ...
  var $_1h25msivjeajok0r = { register: register$1 };

  PluginManager.add('searchreplace', function (editor) {
    var currentIndexState = Cell(-1);
    $_a6bjjcitjeajok0m.register(editor, currentIndexState);
    $_1h25msivjeajok0r.register(editor, currentIndexState);
    return $_8m015bipjeajok05.get(editor, currentIndexState);
  });
  function Plugin () {
  }
