...
var importcss = (function () {
  ...
  var defaultConvertSelectorToFormat = function (editor, selectorText) {
    ...
    if ($_byodlle5jeajojd2.shouldMergeClasses(editor) !== false) {
  ...
  var isExclusiveMode = function (editor, group) {
    return group === null || $_byodlle5jeajojd2.shouldImportExclusive(editor) !== false;
  };
  ...
  var convertSelectorToFormat = function (editor, plugin, selector, group) {
    ...
    } else if ($_byodlle5jeajojd2.getSelectorConverter(editor)) {
      selectorConverter = $_byodlle5jeajojd2.getSelectorConverter(editor);
    } else {
  ...
  var setup = function (editor) {
    editor.on('renderFormatsMenu', function (e) {
      ...
      var selectorFilter = compileFilter($_byodlle5jeajojd2.getSelectorFilter(editor)), ctrl = e.control;
      var groups = compileUserDefinedGroups($_byodlle5jeajojd2.getCssGroups(editor));
      ...
      if (!$_byodlle5jeajojd2.shouldAppend(editor)) {
        ctrl.items().remove();
      }
      Tools.each(getSelectors(editor, e.doc || editor.getDoc(), compileFilter($_byodlle5jeajojd2.getFileFilter(editor))), function (selector) {
  ...
  var get = function (editor) {
    var convertSelectorToFormat = function (selectorText) {
      return $_85h3iie0jeajojcv.defaultConvertSelectorToFormat(editor, selectorText);
    };
    return { convertSelectorToFormat: convertSelectorToFormat };
  };
  var $_dmkhf9dzjeajojcs = { get: get };

  PluginManager.add('importcss', function (editor) {
    $_85h3iie0jeajojcv.setup(editor);
    return $_dmkhf9dzjeajojcs.get(editor);
  });
  function Plugin () {
  }
